...
int main() {
  ...
  struct rlimit max_unsigned = {.rlim_cur = LONG_MAX,
  ...
  // Decrementing the initial limit from RLIM_INFINITY will cause the next write
  // to fail. I think the problem is that this is being treated as a signed
  // number somewhere, with RLIM_INFINITY (0xffff_ffff_ffff_ffff == -1)
  // special-cased. Decrementing causes it to be interpreted as -2 instead of a
  // large positive number.
  struct rlimit decremented = {.rlim_cur = initial.rlim_cur - 1,
